...
func RunOnce(ctx context.Context, client *config.Client, publisher *mq.Publisher) error {
	...
	invalidIDs := make([]string, 0, len(agendas))
	numericID := regexp.MustCompile(`^\d+$`)
	for _, agenda := range agendas {
		if agenda.UcaID == "" {
			continue
		}
		if !numericID.MatchString(agenda.UcaID) {
			invalidIDs = append(invalidIDs, agenda.UcaID)
			continue
		}
	...
	if len(resourceIDs) == 0 {
		if len(invalidIDs) > 0 {
			return fmt.Errorf("no numeric agenda ids available; check Config agendas ucaId values (examples: %s)", strings.Join(invalidIDs, ", "))
		}
	...
	if resp.StatusCode < http.StatusOK || resp.StatusCode >= http.StatusMultipleChoices {
		...
		return fmt.Errorf("unexpected ical status code: %d (%s)", resp.StatusCode, strings.TrimSpace(string(bodyBytes)))
	}
